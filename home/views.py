...
from.models import AppInfo,Category,Contact,Product
...
def pay(request):
    if request.method =='POST':
        api_key = 'sk_test_fb9d42abc43feff0e406edfba01c98ca82b9a81d' #secret key from paystack
        curl = 'https://api.paystack.co/transaction/initialize' #paystack call url
        cburl = 'http://44.203.116.217/callback' #payment confirmation page
        ref = str(uuid.uuid4()) #reference id required by paystack as an additional order member
